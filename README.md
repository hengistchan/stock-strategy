...
MVP 一次只加载一种 K 线周期和交易时段。策略传给 `ma()`、`bar_close()` 等 API 的 `BarType` / `THType` 必须与回测输入一致，并且手册行情与指标 API 要求 `QFQ`；不一致时会明确终止回测，不会静默按另一种周期计算。需要让策略跟随 Web 所选数据契约时，可使用项目扩展 `current_bar_type()` 和 `current_session_type()`；它们只读取本次 OpenD 输入，不进行重采样或时段过滤。
