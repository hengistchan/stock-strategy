...
参数可通过 `label_i18n` 和 `description_i18n` 提供本地化文案；未声明对应语言时，Web 会回退到基础 `label` / `description`，因此已有策略保持兼容。

股票策略可以同时使用多个 K 线周期。Web 会识别脚本中的最细 `BarType` 作为 OpenD 驱动周期，运行时仅用当时已经到达的驱动 K 线增量构造更粗周期，避免未来数据；美股策略仍需统一一个 `THType`。手册行情与指标 API 要求 `QFQ`，错误的驱动周期、时段或复权方式会在任务创建或执行前明确终止。项目扩展 `current_bar_type()` 和 `current_session_type()` 可读取本次 OpenD 驱动契约。
