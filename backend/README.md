...
.venv/bin/python -m stock_strategy.doctor
...
`GET /api/diagnostics` 与 `stock-doctor` 共用 `DiagnosticsService`，检查 Python、Futu SDK、工作区目录、OpenD TCP 和股票目录读取权限。Web 状态面板只展示这份真实结果，不自行推断行情权限。
